...
import type { FinanceSourcesStatusResponse, RecurringSuggestion } from "@/lib/finance/types-client";
import type {
  AnalyticsPayload,
  ChatMessageRow,
  DashboardPayload,
...
export const api = {
  ...
  tradingTriggers: (c: ApiConfig, symbol?: string) =>
    apiFetch<TriggerRow[]>(c, `/trading/triggers?limit=100${symbol ? `&symbol=${encodeURIComponent(symbol)}` : ""}`),
  tradingAnalytics: (c: ApiConfig, scope: { execution: string; track: string; days: number }) =>
    apiFetch<AnalyticsPayload>(
      c,
  ...
  tradingUniverse: (c: ApiConfig) =>
    apiFetch<{ universe: UniverseRow[]; calendar: Array<{ id: string; kind: string; date: string; symbol: string | null; note: string | null; source: string }> }>(
      c,
      "/trading/universe"
    ),
  ...
  tradingChat: (c: ApiConfig) => apiFetch<ChatMessageRow[]>(c, "/trading/chat"),
  sendTradingChat: (c: ApiConfig, message: string) =>
    apiFetch<ChatMessageRow>(c, "/trading/chat", { method: "POST", body: { message } }),
