...
import type { HistoryTrendMetric, HistoryTrends } from "@/lib/finance/history-trends";
...
function SeriesChart({ trends, locale }: { trends: HistoryTrends; locale: string }) {
  const { t } = useI18n();
  const c = useColors();
      ...
      {trends.series.map((point) => {
        ...
        return (
          <View key={point.month}>
            ...
            <View style={{ flexDirection: "row", gap: 4, height: 6, marginBottom: 2 }}>
  ...
  return (
    <>
      ...
      <Card>
        ...
        <SeriesChart trends={trends} locale={locale} />
      </Card>
