...
- [x] הגדרות: החלפת שפה (עברית/English), סטטוס סנכרון יומן, כתובת שרת, התנתקות
- [x] הוספה מהירה לכל סוגי הפריטים — פאנל צ'יפים במסך הבית (מקביל לתפריט ➕ באתר;
      כל צ'יפ מנווט למסך המתאים ופותח את טופס ההוספה עם `?add=…`)
