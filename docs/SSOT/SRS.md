...
### NFR-UX-07
Task-source sync upserts in chunks of 100 against the `(source, external_id)` unique constraint rather than one round-trip per task; drafts are de-duplicated by external id first, since a chunk may not touch the same row twice. Daily cron routes declare `maxDuration = 60` because they sync inline with no client waiting.
