...
אסטרטגיות פעילות (שלב בדיקות, חשבון דמו בלבד):
1) intraday (כל 5 דקות, קריפטו): setup על נרות 15 דקות — וויקוף Spring / SOS / LPS ופריצה מהתכווצות, רק בכיוון מגמת 15 דקות (מעל EMA200); כניסה אחרי אישור בנר 5 דקות; סטופ מבני (לפחות 1.2%), יעד ≥2R, BE ב-1R, נגרר אחרי 1.5R, יציאת זמן אחרי 12 שעות. הסוכן מסחר כאן רק מדרג כל עסקה 1–10 עם הסבר — הדירוג לא משפיע על הכניסה, הגודל או היציאה, והוא נמדד מול ה-R בפועל.
בנוסף: כפתור "חפש עסקה" באפליקציה — סורק את כל היקום התוך-יומי (קריפטו מעל $10M ביום + כ-150 מניות נזילות בשעות המסחר), הסוכן מתכנן כניסה/סטופ/יעד ומדרג, והמשתמש נכנס בלחיצה (עסקאות strategy_version=manual).
2) daily_trend (פעם ביום): סגירה בשיא 100 יום מעל SMA200, סטופ 3×ATR, נגרר 3×ATR, יציאה בשבירת שפל 20 יום.
סריקת ה-4h (v2) כבויה; פוזיציות v2 פתוחות, אם יש, עדיין מנוהלות.
