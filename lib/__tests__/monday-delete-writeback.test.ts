...
describe("Monday delete writeback helpers", () => {
  ...
  it("prefers permission error when complete and archive both fail", () => {
    const completeErr = new MondayGraphqlError(403, "forbidden", ["Not authorized"]);
    const archiveErr = new MondayGraphqlError(500, "server", ["Internal error"]);
  ...
  it("still reports unexpected Monday failures", () => {
    const err = new MondayGraphqlError(500, "server", ["Internal server error"]);
